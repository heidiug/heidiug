...
* **项目描述:** 这是一个基于角色的（RBAC）中后台管理系统...
* **我的职责:**
    * 主导实现了**动态路由**和**按钮级权限**方案，通过 `router.addRoute()` 动态加载用户菜单。
    * 使用 `axios` 拦截器封装了 **Token 认证**和**无感刷新**逻辑。
    * 封装了多个业务组件（如 `ProTable`, `UploadModal`），提升了团队复用效率。
* **[🔗 在线访问] | [🔗 仓库源码]**
